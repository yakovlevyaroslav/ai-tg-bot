...
#   ./release.sh --systemd        # systemd вместо PM2
#   ./release.sh --pm2            # принудительно PM2
...
detect_runner() {
  ...
  echo "error: neither PM2 (ai-tg-bot) nor systemd (ai-tg-bot.service) found." >&2
  echo "  PM2:    pm2 start ecosystem.config.cjs" >&2
  echo "  systemd: sudo bash deploy/install-systemd.sh" >&2
  exit 1
}
